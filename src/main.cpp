...
const string strMessageMagic = "Duckduckcoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/45.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
{
    int64 nSubsidy = .000121 * COIN;

    if(nHeight > 0 && nHeight < 2)
        return(24.2 * COIN);
        //return (4.84 * COIN);

    // 797400 total blocks until 'mined out'
    if(nHeight > 797400)
        return nFees;

    if( nHeight % 1210==0 && nHeight < 300000){
        nSubsidy = .00121 * COIN;
    }else if(nHeight==400121){
        nSubsidy = .0121 * COIN;
    }else if(nHeight==500121){
        nSubsidy = .0121 * COIN;
    }else if(nHeight==600121){
        nSubsidy = .0121 * COIN;
    }else if(nHeight==700121){
        nSubsidy = .01052700331 * COIN;
    }else{
        nSubsidy = .000121 * COIN;
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 60; // 21 minutes
static const int64 nTargetSpacing = 40;
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
     ...
     if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
         ...
         for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                 ...
                 if (LatestBlockTime < BlockReading->GetBlockTime()) {
                         if (BlockReading->nHeight > 116500) LatestBlockTime = BlockReading->GetBlockTime();
                 }
                 PastRateActualSeconds                   = LatestBlockTime - BlockReading->GetBlockTime();
 
                PastRateTargetSeconds                        = TargetBlocksSpacingSeconds * PastBlocksMass;
                 PastRateAdjustmentRatio                        = double(1);
                 if (BlockReading->nHeight > 116500) {
 ...
 unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
 {
         ...
         if (fTestNet) {
                 ...
                 if (pindexLast->nHeight+1 >= 60) { DiffMode = 2; }
         }
...
void ThreadScriptCheck() {
    RenameThread("duckduckcoin-scriptch");
...
void SetGenesisHash() {
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x6c2d2424 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x6c, 0x2d, 0x24, 0x24 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DuckduckcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DuckduckcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("DuckduckcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("duckduckcoin-miner");
    ...
    try { forloop {
        ...
        printf("Running DuckduckcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DuckduckcoinMiner terminated\n");
        throw;
    }
}
