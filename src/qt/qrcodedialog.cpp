...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("DUCK")
    updateDisplayUnit();
}
...
{
    QString ret = QString("duckduckcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DUCK unit input in lnReqAmount, we generate the URI with DUCK as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DUCK, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
